...
__global__ 
void parallel_eval(Individual** pop,int pop_size,char* target){
    ...
    for (i = 0; i < MAX_GENERATION; i++)
        {
            ...
            if(parallel){ 
                ...
                parallel_eval<<< pop_size/numThread , numThread >>>(dev_pop,pop_size,target);
